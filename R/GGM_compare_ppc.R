...
GGM_compare_ppc <- function(Y_g1,
  ...
  if(is.null(contrast) & is.null(compare_all)) stop("contrast or compare all must be specified")

  if(!is.null(contrast) & !is.null(compare_all)) stop("please specifiy only a contrast or compare all")
  ...
  if(!is.null(contrast)){
    ...
    check_contrast <- unlist(regmatches(contrast, gregexpr("[[:digit:]]+", contrast)))

    # invalid
    if(isFALSE(any(check_contrast < groups))) stop("invalid contrast. please respecify")

    if(any(check_contrast > groups)) stop("invalid contrast. please respicify")

    contrast_2 <- lapply(1:length(contrast), function(x) BGGM:::contrast_helper(contrast[x]))

    if(length(unique(contrast_2)) != length(contrast)) stop("redundant contrast. please respecify.")


  }

  # compare all ?
  if(isTRUE(compare_all)) {

    contrast2 <- combn(1:groups, 2)
    contrast2 <-  lapply(1:ncol(contrast2), function(x)  paste("Y", t(contrast2)[x,], sep = "_g", collapse = "_vs_"))
  }
  ...
  for(i in 1:length(contrast2)){
    ...
    # returned object
    results[[i]] <- list(p_value_df = p_value_df,
                         obs_jsd = obs_jsd,
