...
Note that `x` takes the matrix and `f` can return either a single number of a number for each node. The next step is to fit the model and compute the network statistics
...
The function `f` is provided to `FUN` and `communities` is passed to `brigde` via `...`. The results can 
be printed
