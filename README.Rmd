...
The predictive check method uses Jensen-Shannon divergence (symmetric KL divergence) or the Sum of squared error (for the partial correlation matrices) to compare groups [@williams2020comparing].

In this first example, personality networks are compared for males and females.
...
```{r, echo=FALSE, warning = FALSE}
load(file = "readme_models/fit_ppc1.rda")
fit <- fit_ppc1
```
...
Notice that `analytic = TRUE` is being used, which is needed in this case because two graphs are 
estimated for each iteration (or draw from the posterior predictive distribution). The next step is to compute the observed Hamming distance
...
```{r, echo=FALSE, warning = FALSE}
load(file = "readme_models/global_hd.rda")
fit <- global_hd
```
...
fit
```
In this case, the *p*-value is 0.556 which does not indicate that the groups are different for this test-statistic. This may seem contradictory to the previous results, but it is important to note that
Hamming distance asks a much different question related to the adjacency matrices (no other
information, such as edge weigths, is considered). Note that `FUN` only requires that the user defined function accepts to data sets as the input and returns a single number (the chosen test-statistic).
