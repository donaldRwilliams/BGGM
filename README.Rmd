...
The returned object can also be plotted, which allows for visualzing the posterior uncertainty interval for each relation. An example is provided below in [Posterior uncertainty intervals](#posterior-uncertatiny). 
The partial correlation matrix is accesed with 
...
simple solution provides competivie performance with "state-of-the-art" methods, assuming that
...
relation; and (3) a negative relation. When using `plot(E)`, there is a network plot for each hypothesis.
...
#### Confirmatory
A central contribution of **BGGM** is confirmatory hypothesis testing. By this
we are referring to testing expectations, as opposed to feeding the data to, say, `estimate`,
