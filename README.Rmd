...
With the regression coefficients in-hand, it is then possible to compute $R^2$ for each node in the network. Similar approaches are sometimes used in the social-behavioral sciences. Here the GGMs are often estimated with $\ell_1$-regularization and the reported $R^2$ is a point estimate. This is problematic, because it can be misleading to note that one node has higher $R^2$ than another when there is not a measure of uncertainty. The present methods overcome this limitation with Bayesian $R^2$ that is described [here](http://www.stat.columbia.edu/~gelman/research/unpublished/bayes_R2.pdf)
...
# knit table
knitr::kable(r2$summary_error[1:5,])
```

Note that the R package mgm can also compute $R^2$, but only provides point estimates. It would be possible to apply the bootstrap. However, then $R^2$ would not be conditional on the fitted model and posterior distributions but instead would capture sampling variability.
