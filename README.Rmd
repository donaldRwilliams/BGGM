...
Note that the developmental version is recommended (and required to run the following examples), due to the algorithms being written in `c++` and 
...
discuss the proposed changes there. All code *must* be written in base `R`, `c ++`, 
or `fortran` (preferably `c ++`). 
