...
These methods allow for gaining evidence for both conditional dependence ($\rho \neq 0$) and independence ($\rho = 0$). Note that GGMs are often thought to characterize conditional independence structures, but evidence for the null hypothesis of no effect is not (typically) assessed. The following uses the Bayes factor to assess evidence for the null vs. alternative hypothesis.
...
With the regression coefficients in-hand, it is then possible to compute $R^2$ for each node in the network. Similar approaches are sometimes used in the social-behavioral sciences. Here the GGMs are often estimated with $\ell_1$-regularization and the reported $R^2$ is a point estimate. This is problematic, because it can be misleading to note that one node has higher $R^2$ than another when there is not a measure of uncertainty. The present methods overcome this limitation with computing Bayesian $R^2$ that is described ([here](http://www.stat.columbia.edu/~gelman/research/unpublished/bayes_R2.pdf))

JAGS will need to be installed to estimate this model ([link](https://sourceforge.net/projects/mcmc-jags/files/))


The following code fits the model, then selects the graph, and finally computes Bayesian $R^2$ conditional on the fitted model.
...
# knit table
knitr::kable(r2$summary_error)
```

Note that the R package mgm can also compute $R^2$, but only provides point estimate. It would be possible to apply the bootstrap. However, then $R^2$ would not be conditional on this fitted model and posterior distributions but instead would capture sampling variability.
