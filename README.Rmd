...
Note that currently all of the methods are being extended to ordinal, binary, and mixed data (as of April 2020). Further, all the functions are being changed to `c++`. These are much more effecient, in some case reducing the computing time from 20 seconds to 1 second. Hence, the developmental version is a work in progress. We hope to have version `2.0.0` of BGGM submitted to CRAN by the end of May, 2020.
...
```
Note that `A1--A2` denotes the partial correlation between variables A1 and A2. Any number of hypothesis can be tested. They just need to be separated by a semi-colon, e.g., `hypothesis = c(A1--A2 > 0; A1--A2 = 0)`, which also demonstrates that it is possible to simultaneously test both inequality (`>` or `<`) and equality (`=`) restrictions.
