...
With the regression coefficients in-hand, it is then possible to compute $R^2$ for each node in the network. Similar approaches are sometimes used in the social-behavioral sciences. Here the GGMs are often estimated with $\ell_1$-regularization and the reported $R^2$ is a point estimate. This is problematic, because it can be misleading to note that one node has higher $R^2$ than another when there is not a measure of uncertainty. The present methods comptue Bayesian $R^2$ that is described [here](http://www.stat.columbia.edu/~gelman/research/unpublished/bayes_R2.pdf)
