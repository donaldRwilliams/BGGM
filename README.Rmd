...
plt_4C
```
Here it can be seen that the Bayes factor is the ratio of density evaluated at 0. In this case, there is evidence for $\mathcal{H}_0$ ($BF_{01} \approx 33$). This plotting option  may be useful for undertanding (and describing) the Bayes factor approach for selecting the graph (or more generally evaluated partial correlations). 
...
$H_1: \rho_{{ij}} < 0$ can be testing by changing `alternative = greater` to `alternative = less`.
