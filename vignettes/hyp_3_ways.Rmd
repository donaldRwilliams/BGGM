...
```
For an example, I used the default which is to assume the data is Gaussian. This can be chaged with `type = ` either `binary`, `ordinal`, or `mixed`.
...
The above approaches cannot provide evidence that the sum is equal. In other workds, just because there was 
not a difference, this does not provide evidence for equality. The Bayes factor methods allow for formally 
asssessing the equality model, that is,
...
where $\matchal{H}_3$ is the complement and can be understood as neither the fist or second hypothesis.
...
Note the `g1` indicates the group and `;` seperates the hypotheses.
...
# Conclusion
Three approaches for testing the same hypothesis were demonstrated in this vignette--any hypothesis can be tested in 
**BGGM** and in several ways. 
