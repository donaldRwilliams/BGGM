...
computing custom network statistics (e.g., centrality). The new funciton is
...
### Fit Model
Because the data is ordinal, semi-parametric copula model is fitted [@hoff2007extending]. 
In **BGGM**, this is `type = mixed` which is kind of a misnomer because the data do not 
have to be "mixed" (continuous and disrete). Note that I fit the model only once which 
highlights that only the posterior samples are needed to compute any network statistic based
on a partial correlation matrix.
...
The option `select = FALSE` indicates to compute the statistics from the partial correlation matrices (nothing set to zero). This can be changed by with `setting = TRUE`. Internally, each of the sampled
