...
users to **compare networks in any way the want**. 
...
1. Note that only the default in **BGGM** have been shown to have nominal error rates. However, there is   a proof in X that suggests the error rate cannot be larger than $2\alpha$ [@meng1994posterior], and, futher, a predictive check is typically below $\alpha$ [i.e., a tendency to be conservative, @gelman2013two]. 
...
## Conclusion
These example certainly open the door for tailoring   the test-statisitc to ask specific question of interest (which I would hope is not the same for each network analysis). And for methods focused researchers perhaps simulations comparing these, among other options, would be a very nice contribution indeed.
